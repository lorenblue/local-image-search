...
CLIP_VECTOR_TABLE_NAME = "clip_image_embeddings"
...
def init_db(conn: sqlite3.Connection) -> None:
    conn.executescript(
        ...
        CREATE TABLE IF NOT EXISTS images (
            ...
            clip_indexed_at REAL NOT NULL DEFAULT 0,
            thumbnail_path TEXT,
    ...
    _add_column_if_missing(conn, "images", "clip_model", "TEXT NOT NULL DEFAULT ''")
    _add_column_if_missing(conn, "images", "clip_indexed_at", "REAL NOT NULL DEFAULT 0")
...
def needs_indexing(
    ...
    vector_exists_sql = _vector_exists_sql(conn, VECTOR_TABLE_NAME)
    row = conn.execute(
        f"""
...
def ensure_vector_table(conn: sqlite3.Connection) -> None:
    _ensure_vector_table(conn, VECTOR_TABLE_NAME, VECTOR_DIMENSIONS)


def ensure_clip_vector_table(conn: sqlite3.Connection) -> None:
    _ensure_vector_table(conn, CLIP_VECTOR_TABLE_NAME, CLIP_VECTOR_DIMENSIONS)
...
def upsert_image_embedding(
    ...
    _validate_embedding_dimensions(embedding, VECTOR_DIMENSIONS)
    conn.execute(
...
def needs_clip_indexing(
    conn: sqlite3.Connection,
    image: ImageFile,
    clip_model: str,
) -> bool:
    vector_exists_sql = _vector_exists_sql(conn, CLIP_VECTOR_TABLE_NAME)
    row = conn.execute(
        f"""
        SELECT file_size, modified_at, clip_model,
               {vector_exists_sql} AS has_clip_embedding
        FROM images
        WHERE path = ?
        """,
        (str(image.path),),
    ...
    return (
        row["file_size"] != image.file_size
        or row["modified_at"] != image.modified_at
        or row["clip_model"] != clip_model
        or not row["has_clip_embedding"]
    )
...
def upsert_clip_image_embedding(
    conn: sqlite3.Connection,
    image_id: int,
    clip_model: str,
    embedding: list[float],
) -> None:
    _validate_embedding_dimensions(embedding, CLIP_VECTOR_DIMENSIONS)
    conn.execute(
        f"DELETE FROM {CLIP_VECTOR_TABLE_NAME} WHERE rowid = ?",
        (image_id,),
    )
    conn.execute(
        f"INSERT INTO {CLIP_VECTOR_TABLE_NAME} (rowid, embedding) VALUES (?, ?)",
        (image_id, serialize_embedding(embedding)),
    )
    conn.execute(
        """
        UPDATE images
        SET clip_model = ?, clip_indexed_at = ?
        WHERE id = ?
        """,
        (clip_model, time.time(), image_id),
    )
...
def search_indexed_images(
    ...
    _validate_embedding_dimensions(query_embedding, VECTOR_DIMENSIONS)
    if not vector_table_exists(conn, VECTOR_TABLE_NAME):
        return []
    rows = conn.execute(
        f"""
...
def search_clip_images(
    ...
    _validate_embedding_dimensions(query_embedding, CLIP_VECTOR_DIMENSIONS)
    if not vector_table_exists(conn, CLIP_VECTOR_TABLE_NAME):
        return []
    rows = conn.execute(
        f"""
        SELECT images.id, images.path, images.file_name, images.file_size,
               ...
               matches.distance,
               (1.0 - ((matches.distance * matches.distance) / 2.0)) AS score
        FROM {CLIP_VECTOR_TABLE_NAME} AS matches
        JOIN images ON images.id = matches.rowid
        WHERE matches.embedding MATCH ?
          AND matches.k = ?
          AND images.clip_model = ?
        ORDER BY matches.distance
        """,
        (serialize_embedding(query_embedding), limit, clip_model),
    ).fetchall()
    return [
        SearchResult(
            image=_row_to_indexed_image(row),
...
def delete_missing_paths(conn: sqlite3.Connection, seen_paths: Iterable[Path]) -> int:
    ...
    for row in rows:
        if row["path"] not in seen:
            if vector_table_exists(conn, VECTOR_TABLE_NAME):
                conn.execute(
                    f"DELETE FROM {VECTOR_TABLE_NAME} WHERE rowid = ?",
                    (row["id"],),
                )
            if vector_table_exists(conn, CLIP_VECTOR_TABLE_NAME):
                conn.execute(
                    f"DELETE FROM {CLIP_VECTOR_TABLE_NAME} WHERE rowid = ?",
                    (row["id"],),
                )
...
def count_clip_searchable_images(
    conn: sqlite3.Connection,
    clip_model: str,
) -> int:
    if not vector_table_exists(conn, CLIP_VECTOR_TABLE_NAME):
    ...
    row = conn.execute(
        f"""
        SELECT COUNT(*) AS count
        FROM {CLIP_VECTOR_TABLE_NAME}
        JOIN images ON images.id = {CLIP_VECTOR_TABLE_NAME}.rowid
        WHERE images.clip_model = ?
        """,
        (clip_model,),
    ).fetchone()
    return int(row["count"])
...
def _vector_exists_sql(conn: sqlite3.Connection, table_name: str) -> str:
    if not vector_table_exists(conn, table_name):
        return "0"
    return (
        "EXISTS ("
        f"SELECT 1 FROM {table_name} "
        f"WHERE {table_name}.rowid = images.id"
        ")"
    )
...
def vector_table_exists(conn: sqlite3.Connection, table_name: str) -> bool:
    row = conn.execute(
        """
        SELECT 1
        FROM sqlite_master
